...
The attribute `exts` defines a list of file extensions which should be associated with that language.
...
The very basic one is the function `dataToAstSimpl :: Data t => t -> Ast` transforming an arbitrary value whose type implements class `Data` into our internal type `Ast` by just printing the constructors and storing them in a tree.
